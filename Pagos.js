...
function paso1_registrarRegistro(datos) {
  ...
  try {
    ...
    const infoPrecioPrincipal = obtenerPrecioYConfiguracion(datos, hojaConfig, indicePrecioAplicar, totalHijos);
    // (FIN MODIFICACIÓN)
    ...
    if (datos.metodoPago === 'Pago en Cuotas' && datos.esPreventa !== true && datos.esPreventa !== 'true') {
      datos.montoAPagar = 0;
    } else {
    ...
    if (datos.esPreventa === true || datos.esPreventa === 'true') {
      datos.metodoPago = datos.metodoPago.includes('Efectivo') ? 'Pago Efectivo (Adm del Club)' : 'Transferencia';
...
function actualizarDatosHermano(datos) {
  ...
  try {
    ...
    const infoPrecio = obtenerPrecioYConfiguracion(datos, hojaConfig, infoConteo.indice, infoConteo.total);
    ...
    hojaRegistro.getRange(fila, COL_PRECIO).setValue(precio).setNumberFormat(currencyFormat); // AE (Precio Individual)
    ...
    // (MODIFICADO v19) Aplicar valor Y formato
    hojaRegistro.getRange(fila, COL_MONTO_A_PAGAR).setValue(montoAPagarFinal).setNumberFormat(currencyFormat); // AK (0 o Total Individual)
    
    
    // (MODIFICADO v17) Corrección de bug 'valorCuota is not defined'
    if (datos.cantidadCuotas === 3 && datos.valorCuota > 0) {
      // (MODIFICADO v19) Aplicar valor Y formato
      hojaRegistro.getRange(fila, COL_CUOTA_1).setValue(datos.valorCuota).setNumberFormat(currencyFormat); // AF
      hojaRegistro.getRange(fila, COL_CUOTA_2).setValue(datos.valorCuota).setNumberFormat(currencyFormat); // AG
      hojaRegistro.getRange(fila, COL_CUOTA_3).setValue(datos.valorCuota).setNumberFormat(currencyFormat); // AH
    }
