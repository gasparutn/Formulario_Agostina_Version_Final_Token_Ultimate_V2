 ...
 */
function obtenerPrecioYConfiguracion(datos, hojaConfig, indiceHijo = 0, totalHijos = 1) {
  ...
  Logger.log(`Calculando precio para indiceHijo: ${indiceHijo}, totalHijos: ${totalHijos}. Jornada=${jornada}, Metodo=${metodoPago}, Socio=${esSocio}, esPreventa=${esPreventa}`);
  ...
  try {
    ...
    if (colLetra && rowNum > 0) {
      ...
      Logger.log(`Obteniendo precio de la celda de Config: ${celdaPrecio}. Es valor/cuota? ${precioEsCuota}`);
  ...
  Logger.log(`Precio Total (AE) [Individual]: ${precioTotal}, Monto a Pagar (AK): ${montoAPagarInicial}, Valor Cuota (AF/AG/AH): ${valorCuota}, Cuotas: ${cantidadCuotas}`);
...
function _obtenerIndiceHijo(hojaRegistro, fila) {
  try {
    ...
    if (totalMiembros <= 0) {
      Logger.log(`Error en _obtenerIndiceHijo: Se encontró vínculo ${vinculoPrincipal} pero el conteo es 0.`);
    ...
    Logger.log(`_obtenerIndiceHijo: Fila ${fila} pertenece a familia ${vinculoPrincipal}. Miembros totales: ${totalMiembros}. Índice de precio aplicado: ${indicePrecio}`);
